...
<body>
	...
	<form:form id="searchForm" modelAttribute="org" action="${ctx}/sys/user/list" method="post" class="breadcrumb form-search ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="org">
			<tr>
				...
				<td><a href="${ctx}/sys/user/form?id=${org.id}">${org.loginName}</a></td>
				<td>${org.name}</td>
				<td>${org.phone}</td>
				<td>${org.mobile}</td><%--
				<td>${user.roleNames}</td> --%>
				<shiro:hasPermission name="sys:user:edit"><td>
    				<a href="${ctx}/sys/user/form?id=${org.id}">修改</a>
					<a href="${ctx}/sys/user/delete?id=${org.id}" onclick="return confirmx('确认要删除该用户吗？', this.href)">删除</a>
