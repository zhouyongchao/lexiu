...
public class SystemService extends BaseService implements InitializingBean {
	 ...
	 * @return
	 */
	public Org getUserByLoginName(String loginName) {
		return UserUtils.getByLoginName(loginName);
	}
	
	public Page<Org> findUser(Page<Org> page, Org user) {
	 ...
	 * @return
	 */
	public List<Org> findUser(Org user){
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(), "o", "a"));
		List<Org> list = userDao.findList(user);
		return list;
	}
	...
	public List<Org> findUserByOfficeId(String officeId) {
		List<Org> list = (List<Org>)CacheUtils.get(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + officeId);
		if (list == null){
			Org user = new Org();
	...
	public void saveUser(Org user) {
		if (StringUtils.isBlank(user.getId())){
			user.preInsert();
			userDao.insert(user);
		}else{
			// 清除原用户机构用户缓存
			Org oldUser = userDao.get(user.getId());
			// 更新用户数据
			user.preUpdate();
			userDao.update(user);
		}
...
//			systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void updateUserInfo(Org user) {
		user.preUpdate();
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void deleteUser(Org user) {
		userDao.delete(user);
		// 同步到Activiti
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void updateUserLoginInfo(Org user) {
		// 保存上次登录信息
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public Boolean outUserInRole(Role role, Org user) {
	...
	@Transactional(readOnly = false)
	public Org assignUserToRole(Role role, Org user) {
		if (user == null){
			return null;
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (isSynActivitiIndetity){
			...
			if (userList.size() == 0){
			 	Iterator<Org> users = userDao.findAllList(new Org()).iterator();
			 	while(users.hasNext()) {
	...
	private void saveActivitiGroup(Role role) {
		...
		// 创建用户与用户组关系
		List<Org> userList = findUser(new Org(new Role(role.getId())));
		for (Org e : userList){
	...
	private void saveActivitiUser(Org user) {
		if (!Global.isSynActivitiIndetity()){
			return;
		}
	...
	private void deleteActivitiUser(Org user) {
		if (!Global.isSynActivitiIndetity()){
			return;
		}
		if(user!=null) {
